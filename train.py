...
def train(model, train_loader, train_images, e, obs_len, pred_len, batch_size, params, gt_template, device, input_template, optimizer, criterion, dataset_name, homo_mat):
	...
	for batch, (trajectory, meta, scene) in enumerate(train_loader):
		...
		for i in range(0, len(trajectory), batch_size):
			...
			observed_map = torch.stack(observed_map).reshape([-1, obs_len, H, W])					     # 4 x 8 x H x W
			...
			gt_future_map = torch.stack(gt_future_map).reshape([-1, pred_len, H, W])                     # 4 x 12 x H x W

			gt_waypoints = gt_future[:, params['waypoints']]
			gt_waypoint_map = get_patch(input_template, gt_waypoints.reshape(-1, 2).cpu().numpy(), H, W)
			gt_waypoint_map = torch.stack(gt_waypoint_map).reshape([-1, gt_waypoints.shape[1], H, W])    # 4 x 1 x H x W
			...
			# Forward pass
			# Calculate features
			features = model.pred_features(feature_input)      	# encoder 的6个feature
			...
			with torch.no_grad():
				...
				# gt_future & pred_traj are both [4, 12, 2]
