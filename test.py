...
def evaluate(model, val_loader, val_images, num_goals, num_traj, obs_len, batch_size, device, input_template, waypoints, resize, temperature, use_TTST=False, use_CWS=False, rel_thresh=0.002, CWS_params=None, dataset_name=None, homo_mat=None, mode='val'):
	...
	with torch.no_grad():
		...
		for trajectory, meta, scene in val_loader:
			...
			scene_image = model.segmentation(scene_image)              							# shape : [1, 6, H, W]
			...
			for i in range(0, len(trajectory), batch_size):
				...
				observed_map = get_patch(input_template, observed, H, W)						# 以轨迹点为中心算距离 做法：在input_template中心点附近截取patch
				observed_map = torch.stack(observed_map).reshape([-1, obs_len, H, W])			# shape: [batch_size, obs_len, H, W]

				#TODO gt_future算指标不需要
				gt_future = trajectory[i:i+batch_size, obs_len:].to(device)
				semantic_image = scene_image.expand(observed_map.shape[0], -1, -1, -1)			# shape: [batch_size, 6, H, W]

				# Forward pass
				# Calculate features
				feature_input = torch.cat([semantic_image, observed_map], dim=1)				# shape: [batch_size, 6+obs_len, H, W]
				features = model.pred_features(feature_input)

				# Predict goal and waypoint probability distributions
				pred_waypoint_map = model.pred_goal(features)									# shape: [batch_size, 12, H, W]
				pred_waypoint_map = pred_waypoint_map[:, waypoints]								# shape: [batch_size, 1, H, W]
	...
	return val_ADE.item(), val_FDE.item(), future_samples/resize, waypoint_samples/resize, observed.reshape(-1, obs_len, 2)/resize
