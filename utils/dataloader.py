...
class SceneDataset(Dataset):
	def __init__(self, data, resize, total_len):
		...
		self.trajectories, self.meta, self.scene_list = self.split_trajectories_by_scene(data, total_len, resize)
	...
	def split_trajectories_by_scene(self, data, total_len, resize):
		trajectories = []
		meta = []
		scene_list = []
		for meta_id, meta_df in tqdm(data.groupby('sceneId', as_index=False), desc='Prepare Dataset'):
			trajectory = meta_df[['x', 'y']].to_numpy().astype('float32').reshape(-1, total_len, 2)
			trajectories.append(trajectory * resize)
			meta.append(meta_df)
