...
class YNet:
	...
	def evaluate(self, data, params, image_path, batch_size=8, num_goals=20, num_traj=1, rounds=1, device=None, dataset_name=None):
		...
		input_template = torch.Tensor(create_dist_mat(size=size)).to(device)   		# distance matrix :  size (1050, 1050) value [0, 2]
		...
		for e in tqdm(range(rounds), desc='Round'):
			#TODO 可视化
			test_ADE, test_FDE, future_samples, waypoint_samples, observed_traj = evaluate(model, test_loader, test_images, num_goals, num_traj,
										  obs_len=obs_len, batch_size=batch_size,
										  device=device, input_template=input_template,
										  waypoints=params['waypoints'], resize=params['resize'],
			...
			# 轨迹-future_samples:[num_goals, batch, pred_len, 2]
			# 终点-waypoint_samples:[num_goals, batch, 1, 2]
			def vis_image(observed_traj, future_samples, num_agents, num_traj, map_path, only_goals=False, output_dir='vis_output/images'):
				r'''
				可视化轨迹并保存为图片
				:param observed_traj: [batch, obs_len, 2]
				:param future_samples: [num_goals, batch, pred_len, 2]
				:param num_agents: int  how many agents to visualize
				:param num_traj: int  how many trajectories for each agent to visualize
				:param only_goals: bool  whether to visualize only the goal points
    			'''
				...
				for i in range(num_agents):
					observed_traj_i = observed_traj[i, :, :]
					ax.scatter(observed_traj_i[:, 0], observed_traj_i[:, 1], c=np.random.rand(8), s=5, alpha=0.5)
					for traj_idx in range(num_traj):
						pred_traj = future_samples[traj_idx, i, :, :].cpu().numpy()
						ax.scatter(pred_traj[:, 0], pred_traj[:, 1], s=5, alpha=0.5)

				output_file = os.path.join(output_dir, 'trajectory_plot_{num_agents}_{num_traj}.png'.format(num_agents=num_agents, num_traj=num_traj))
    
				if not os.path.exists(output_dir):
					os.makedirs(output_dir)

				plt.savefig(output_file, dpi=1000,  bbox_inches='tight')
				print('The result image has been saved in {output_file}.\n'.format(output_file))
				plt.show()

			def vis_video(observed_traj, future_samples, num_agents, num_traj, map_path, only_goals=False, output_dir='vis_output/video'):
				r'''
				可视化轨迹并保存为视频
				:param observed_traj: [batch, obs_len, 2]
				:param future_samples: [num_goals, batch, pred_len, 2]
				...
				# 设置视频帧率和尺寸
				frame_rate = 2.5  # 每秒帧数
				frame_size = (int(map_image.shape[1]), int(map_image.shape[0]))  # 图像尺寸
				video_path = os.path.join(output_dir, 'trajectory_video_{num_agents}_{num_traj}.mp4'.format(num_agents=num_agents, num_traj=num_traj))

				# 创建视频编写器对象
				fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # 编码器
				video_writer = cv2.VideoWriter(video_path, fourcc, frame_rate, frame_size)

				for frame_idx in range(obs_len+pred_len):
					fig, ax = plt.subplots(figsize=(10, 10))
					ax.imshow(map_image)
					if frame_idx < obs_len:
						for i in range(num_agents):
							observed_traj_i = observed_traj[i, :, :]
							ax.scatter(observed_traj_i[:frame_idx, 0], observed_traj_i[:frame_idx, 1], c=np.random.rand(frame_idx), s=5, alpha=0.5)
					else:
						for i in range(num_agents):
							for traj_idx in range(num_traj):
								pred_traj = future_samples[traj_idx, i, :, :].cpu().numpy()
								ax.scatter(pred_traj[:frame_idx-obs_len, 0], pred_traj[:frame_idx-obs_len, 1], s=5, alpha=0.5)
				...
				print('The result video has been saved.\n')

			vis_image(observed_traj, future_samples, num_agents=20, num_traj=1, map_path='test/scene/plat/reference.jpg', only_goals=False, output_dir='vis_output/images')	

			vis_video(observed_traj, future_samples, num_agents=20, num_traj=1, map_path='test/scene/plat/reference.jpg', only_goals=False, output_dir='vis_output/video')
